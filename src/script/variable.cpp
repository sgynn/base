...
//// String ////
String::String(const char* v) : s(0), ref(0) {
	if(v && v[0]) { s=strdup(v); ref=new int(1); }
}
String::String(const char* v, uint l): s(0), ref(0) {
	if(v && l) {
		l = fmin(l, strlen(v));
		if(l>0) {
			s = (char*)malloc(l+1);
...
String::String(String&& v): s(v.s), ref(v.ref) {
	v.s = 0;
	v.ref = 0;
}
...
const String& String::operator=(String&& v) {
	drop();
	s = v.s; ref=v.ref;
	v.s = 0; v.ref=0;
	return *this;
}
const String& String::operator=(const char* v) {
	drop();
	if(v && v[0]) { s=strdup(v); ref=new int(1); }
	return *this;
}
...
String String::operator+(const char* v) const {
	if(!v) return *this;
	if(empty()) return String(v);
	return cat(s, v);
}
String operator+(const char* s, const String& v) {
	if(!s) return v;
	if(v.empty()) return String(s);
	return String::cat(s,v);
}

bool String::operator==(const char* v) const {
	return s==v || (s && v && strcmp(s, v)==0);
}
bool String::operator!=(const char* v) const {
	return !operator==(v);
}

char String::operator[](int i) const { return s[i]; }
char& String::operator[](int i) { makeUnique(); return s[i]; }

StringList  String::split(const char* src, const char* tok, bool keepEmpty, const char* trim) {
	StringList out;
