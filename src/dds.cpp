...
inline DWORD calculateSize(DDS::DDSFormat f, int w, int h, int d) {
	if(f==DDS::DXT3 || f==DDS::DXT5) return ((w+3)/4) * ((h+3)/4) * 16 * d;
	else if(f==DDS::DXT1) return ((w+3)/4) * ((h+3)/4) * 8 * d;
	else return f * w * h * d;
}
}
...
DDS DDS::load(const char* filename) {
	...
	if(header.format.fourCC & DDS_FOURCC) {
		...
		case FOURCC_DXT1: dds.format = DXT1; break;
		case FOURCC_DXT3: dds.format = DXT3; break;
		case FOURCC_DXT5: dds.format = DXT5; break;
		default:
...
void DDS::flipTexture(BYTE* data, int w, int h, int d) {
	...
	// Flipping compressed images is more complicated
		...
		int bx = w/4;
		int by = h/4;
		int bs = format==DXT1? 8: 16;
		int line = bx * bs;

		BYTE* tmp = new BYTE[bs * bx];

		// DXT1 Block { short c0, c1; byte row[4]; }
		// DXT3 Alpha { byte row[4]; }
		// DXT5 ALpha { byte a0, a1, byte row[6]; } // Lookup table is 3bits per pixel
...
void DDS::createDXT5AlphaTable(BYTE a, BYTE b, BYTE* o) {
	o[0] = a;
	o[1] = b;
	if(a>b) {
		o[2] = (a*6 + b*1) / 7;
...
void DDS::decompress(const BYTE* data, int w, int h, BYTE* out) const {
	...
	BYTE c[12];
	const BYTE* cdata = format==DXT1? data: data+8;
	BYTE* cout = out + (format==DXT1? 0: 1);
	...
	if(format == DXT3) {
		...
		for(int x=0; x<bx; ++x) for(int y=0; y<by; ++y) {
			const BYTE* block = data + x * bs + y*line;
			createDXT5AlphaTable(block[0], block[1], c);
			memcpy(&b, block, 8);
			for(int i=0; i<4; ++i) for(int j=0; j<4; ++j) {
				int k = i + j*4;
				int v = (b >> (48 - k*3)) & 7;
