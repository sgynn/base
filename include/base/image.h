...
namespace base {
	class Image {
		...
		int getChannels() const      { constexpr const int c[]={0,1,2,3,4,3,4,4,1,2,1,1,1}; return c[m_format]; }
		int getBytesPerPixel() const { constexpr const int b[]={0,1,2,3,4,0,0,0,0,0,2,2,4}; return b[m_format]; }
		...
		void flip();
		Image convert(Format) const;
		private:
		void flipSurface(byte* data, int w, int h, int d);
		byte* convertSurface(byte* data, int w, int h, int d, Format to) const;
		protected:
		Mode m_mode = FLAT;
